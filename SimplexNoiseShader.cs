...
using GeoClip.Shaders.Textures;
...
namespace GeoClip.Shaders.Examples.Shaders
{
    ...
    public abstract class SimplexNoiseShader: Shader
    {
        ...
        public static readonly int[] Perm =
            {
        ...
        public static readonly int[,] Grad3 =
            {
        ...
        {
            var perm2 = new byte[256, 256, 4];
            for (var y = 0; y < 256; y++)
            {
                for (var x = 0; x < 256; x++)
                {
                    var a = Perm[x] + y;
                    perm2[y, x, 0] = (byte) Perm[a];
                    perm2[y, x, 1] = (byte) Perm[a + 1];

                    var b = Perm[x + 1] + y;
                    perm2[y, x, 2] = (byte) Perm[b];
                    perm2[y, x, 3] = (byte) Perm[b + 1];
                }
            }

            _perm2D = new Texture2D(256, 256, 4, typeof(byte));
            _perm2D.Activate();

            GL.TexSubImage2D(TextureTarget.Texture2D, 0, 0, 0, 256, 256, PixelFormat.Rgba, PixelType.Byte, perm2);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Nearest);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, (int)TextureMagFilter.Nearest);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapS, (int)TextureWrapMode.Repeat);
            GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)TextureWrapMode.Repeat);

            _permGrad3 = new Texture1D(256,  3, typeof(OpenTK.Half));
            var pg3 = Perm.SelectMany(i => new[]
            {
                (float)Grad3[i % 16, 0],
                (float)Grad3[i % 16, 1],
                (float)Grad3[i % 16, 2]
            ...
            _permGrad3.Activate();
            GL.TexSubImage1D(TextureTarget.Texture1D, 0, 0, 256, PixelFormat.Rgb, PixelType.Float, pg3);
